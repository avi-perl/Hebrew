...
@pytest.mark.parametrize("number, expected_output", WITH_GERESH.items())
def test_with_geresh(number, expected_output):
    assert Hebrew.from_number(number, True, True) == Hebrew(expected_output)
    assert number_to_hebrew_string(number, True, True) == expected_output


@pytest.mark.parametrize(
    "number, expected_output",
    [(i[0], i[1].replace("׳", "'").replace("״", '"')) for i in WITH_GERESH.items()],
)
def test_with_english_quote(number, expected_output):
    assert Hebrew.from_number(number, True, False) == Hebrew(expected_output)
    assert number_to_hebrew_string(number, True, False) == expected_output
...
def test_with_no_punctuation(number, expected_output):
    assert Hebrew.from_number(number, False) == Hebrew(expected_output)
    assert number_to_hebrew_string(number, False) == expected_output
