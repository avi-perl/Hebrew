...
def test_as_str():
    for pasuk in perek_aleph_all:
        hs = HebrewString(pasuk)
        assert hs.string == pasuk


def test_without_maqaf():
    assert HebrewString(perek_aleph_all[1]).without_maqaf == perek_aleph_all[1].replace(HebrewString.MAQAF, " ")


def test_strip_non_letters():
    for index, pasuk in enumerate(perek_aleph_all):
        hs = HebrewString(pasuk)
        assert hs.strip_non_letters == perek_aleph_text_only[index]


def test_strip_non_letters_no_maqaf():
    for index, pasuk in enumerate(perek_aleph_all):
        hs = HebrewString(pasuk)
        assert hs.strip_non_letters_no_maqaf == perek_aleph_text_only[index].replace(HebrewString.MAQAF, " ")


def test_strip_trup():
    hs = HebrewString(perek_aleph_all[0])
    assert hs.strip_trup == perek_aleph_nikkud[0]
